...
export function SportsPicksPage() {
  ...
  return <main id="main-content" className="min-h-screen bg-white pb-24 pt-20 text-[#111]">
    <section className="bg-[#F5F5DC] px-5 pb-9 pt-8 sm:px-8 lg:px-5"><div className="mx-auto max-w-[1800px]">
      ...
      <nav aria-label={language === "en" ? "Sports review categories" : "용품 후기 분류"} className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{categories.map((item) => { const Icon = item.icon; const count = item.category ? sportsPicks.filter((pick) => pick.category === item.category).length : sportsPicks.length; const selected = active === item.key; return <button key={item.key} onClick={() => setActive(item.key)} aria-pressed={selected} className={`flex min-h-[5.75rem] items-center gap-4 rounded-2xl border p-4 text-left transition-all active:scale-[.99] ${selected ? "border-black bg-black text-white" : "border-black/10 bg-white hover:border-black/35"}`}><span className="grid h-12 w-12 shrink-0 place-items-center rounded-full" style={{ backgroundColor: item.iconBg, color: item.iconColor }}><Icon className="h-5 w-5" /></span><span><strong className="block text-lg font-black tracking-[-.04em] sm:text-xl">{label(item)}</strong><span className={`mt-1 block font-mono text-xs ${selected ? "text-white/65" : "text-black/45"}`}>{count} {language === "en" ? "picks" : "개"}</span></span></button>; })}</nav>
    </div></section>
